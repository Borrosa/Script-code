...
// Created with      : 婆罗莎自制AOB模板注入 version: %extensionVersion%
<<INFO_END>>
...
if cheatEngineIs64Bit() then
	...
	autoAssemble([[
		...
		define(plusminuslines,18) // 0x18 = 24   //注入时备注的行数

		alloc(newmem,$1000,400000)

		aobscanmodule(Full_Injection_minus128bytes,400000,48 8D 50 80 4C 8D 85 F0 FE FF FF)
		label(code1)
		label(return1)
	...
	autoAssemble([[
		...
		aobscanmodule(AOB_Injection_minus128bytes,400000,48 8D 50 80 4C 8D 85 10 FF FF FF)
		label(code1)
		label(return1)
		...
		aobscanmodule(AOB_Injection_tenLinesBeforeTenLinesAfter,400000,83 E8 0A 8B 95 00 FF FF FF 67 8D 5A 0A)
		label(code3)
		label(return3)
